/**
 * Vercel Serverless Function - Storybook chapter generator
 *
 * Turns a session's notes into an illustrated storybook chapter. Produces:
 *   - a dramatic 4-8 paragraph prose narration (markdown)
 *   - 2-4 scene illustration prompts, each tagged with the featured entities
 *     so the client orchestrator can bake in their cached descriptions
 *   - up to 4 spotlight entities (who mattered most in this session)
 */
...
Produce exactly ${numScenes} scenes. Return ONLY the JSON object.`;
    ...
    console.error('generate-storybook-chapter error:', err);
    return res.status(500).json({
      error: 'Failed to generate storybook chapter',
      message: err.message
    });
  }
}
